...
function getBaseUri(server: Server = 'default', config: Configuration): string {
  if (config.environment === Environment.USServer) {
    if (server === 'default') {
      return 'https://na1.foxitesign.foxit.com/api';
    }
  }
  if (config.environment === Environment.EUServer) {
    if (server === 'default') {
      return 'https://eu1.foxitesign.foxit.com/api';
    }
  }
